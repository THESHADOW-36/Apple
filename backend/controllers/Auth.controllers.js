...
export const Register = asyncHandler(async (req, res, next) => {
    const { firstname, lastname, country, countryCode, dob, email, password, phone } = req.body;
    if (!firstname || !lastname || !country || !countryCode || !dob || !email || !password || !phone) return next(new ErrorResponse("All fields are mandatory...", 401));

    const user = await User.create({ firstname, lastname, country, countryCode, dob, email, password, phone })
